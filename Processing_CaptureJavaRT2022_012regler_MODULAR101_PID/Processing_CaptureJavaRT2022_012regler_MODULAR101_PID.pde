...
void setup()
{
    ...
    algo.startALL();
    maskYellow = new ColorHSV("Yellow", redMask);
...
void draw()
{
    ...
    y = maskYellow.getMask(cam, false);
    ...
    ArrayList<Contour> contours = maskYellow.getContour();
    if (contours.size() > 0) {
...
// eventhandler for OSC messages
void oscEvent(OscMessage theOscMessage) {
    /* checkif theOscMessage has the address pattern we are looking for. */
